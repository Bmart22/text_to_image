...
# Predicts the class of the first 10 example images
...
def main(argv):
    ...
    num_images = 20
    image_names = [ image_ind.iloc[np.random.randint(image_ind.shape[0]),:]["Image"][:-4] for i in range(num_images) ]
    
    # Generate a numerical embedding for the images' captions
    # Embedding is 200 features long
    embedding = gen_embed( model, image_names, master_word_list )
    ...
    # Test the effecitveness of the model for classification
    accuracy, precision, recall = precision_recall( model, num_test_batches, master_word_list, image_ind, image_classes, data_split, "train" )
    print(f"Train Accuracy: {accuracy}")
    print(f"Train Precision: {precision}")
    print(f"Train Recall: {recall}")


    accuracy, precision, recall = precision_recall( model, num_test_batches, master_word_list, image_ind, image_classes, data_split, "test" )
